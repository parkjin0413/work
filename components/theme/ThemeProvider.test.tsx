...
describe("ThemeProvider light/dark toggle", () => {
  ...
  // defaultTheme="light"). Reset storage and the <html> class before each
  ...
  it("라이트에서 다크로 전환하면 html 요소에 dark 클래스가 추가된다", async () => {
    ...
    const toggleButton = await screen.findByRole("button", { name: "다크 모드로 전환" });
  ...
  it("다크에서 라이트로 전환하면 html 요소의 dark 클래스가 제거된다", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "라이트 모드로 전환" }));
